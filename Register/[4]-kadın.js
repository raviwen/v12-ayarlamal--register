...
module.exports.raviwen = async(client, message, args, config) => {
    ...
    if(!k1) return message.channel.send('Kadın **1** Rolü Ayarlı Değil').then(x => x.delete({timeout:2000}))
    const k2 = db.fetch(`kadın2.${message.guild.id}`);
    if(!k2) return message.channel.send('Kadın **2** Rolü Ayarlı Değil').then(x => x.delete({timeout:2000}))
    ...
    await uye.roles.add(k1)
    await uye.roles.add(k2)
    await uye.roles.remove(kayıtsız)
    message.channel.send(new MessageEmbed().setColor('RANDOM').setFooter(`${message.author.username} Adlı yetkilinin toplam \`${reg}\` kaydı bulunuyor.`).setAuthor(message.author.username, message.author.avatarURL({ dynamic: true})).setThumbnail(message.guild.iconURL({ dynamic: true })).setDescription(`${uye} Adlı kişi ${message.author} tarafından <@&${k1}> Rolü verilerek kayıt edildi. \n  Kişinin yeni ismi: \`${İsim}\``))
